...
class DemoLegalAssistant:
    ...
    def _format_demo_response(self, question: str, docs: list) -> str:
        ...
        short_docs = []
        for doc_data in docs[:2]:  # Берем только первые 2 документа
            doc_content = ""
            if isinstance(doc_data, dict) and 'content' in doc_data:
                doc_content = str(doc_data['content'])
            ...
            if len(doc_content) > 150:
                short_doc = doc_content[:150] + "..."
            else:
                short_doc = doc_content
            short_docs.append(short_doc)
        
        if not short_docs:
            found_docs_text = "Не удалось извлечь текст из найденных документов."
        else:
            found_docs_text = "\\n".join([f"• {doc}" for doc in short_docs])
...
[ВАЖНО:] Это демо-версия. Для получения полноценных ответов с анализом ИИ настройте OpenAI API ключ в файле .env
...
"""
    ...
    def _get_no_info_response(self) -> str:
        ...
        return """
[ИЗВИНИТЕ] К сожалению, информация по вашему вопросу не найдена в базе знаний.
...
[ВАЖНО:] Это демо-версия - для полноценной работы настройте API ключи.
"""
...
[ВАЖНО:] Это демо-версия - для полноценной работы настройте OpenAI API ключ.
"""
